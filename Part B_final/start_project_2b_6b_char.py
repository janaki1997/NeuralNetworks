...
def rnn_model(x):
  ...
  cells = [GRU_cell(HIDDEN_SIZE) for _ in range(NUM_LAYER)]
  stacked_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(cells)
  cell_wrapped = tf.contrib.rnn.OutputProjectionWrapper(stacked_rnn_cell, output_size = 1)

  _, encoding = tf.nn.static_rnn(cell_wrapped, char_list, dtype=tf.float32)
...
def main():
  ...
  with tf.Session() as sess:
    ...
    np.savetxt('./Q6b_char_test_accs.txt',np_test_accs)
    np.savetxt('./Q6b_char_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q6b_char.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q6b_char.png')
    ...
    plt.savefig('./Train acc vs Test Acc_Q6b_char.png')
