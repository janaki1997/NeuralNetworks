...
def main():
  ...
  with tf.Session() as sess:
    ...
    np.savetxt('./Q6b_word_test_accs.txt',np_test_accs)
    np.savetxt('./Q6b_word_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q6b_word.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q6b_word.png')
    ...
    plt.savefig('./Train Acc vs Test Acc_Q6b_word.png')
    plt.show()
