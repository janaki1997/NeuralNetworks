...
def word_cnn_model(x):
  ...
  with tf.variable_scope('CNN_Layer1'):
        ...
        logits_dropout = tf.layers.dense(pool2_dropout, MAX_LABEL, activation=None,name="logits_dropout")

  return logits, keep_prob, logits_dropout
...
def main():
  ...
  logits, keep_prob, logits_dropout = word_cnn_model(x)
  ...
  with tf.Session() as sess:
    ...
    np.savetxt('./Q5b_dropout_test_accs.txt',np_test_accs)
    np.savetxt('./Q5b_dropout_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q5b_dropout.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q5b_dropout.png')
    ...
    plt.savefig('./Train Acc vs Test Acc_Q5b_dropout.png')
