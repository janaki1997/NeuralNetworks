...
def rnn_model(x):
  ...
  cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=0.5) #Dropout
  _, encoding = tf.nn.static_rnn(cell, char_list, dtype=tf.float32)

  logits = tf.layers.dense(encoding, MAX_LABEL, activation=None)

  return logits, char_list, output_keep_prob
...
def main():
  ...
  logits, char_list, output_keep_prob = rnn_model(x)
  ...
  with tf.Session() as sess:
    ...
    for e in range(epochs):
        ...
        for start, end in zip(range(0, N+1, batch_size), range(batch_size, N+1, batch_size)):
            sess.run([train_op], {x: randomizedX[start:end], y_: randomizedY[start:end],output_keep_prob:0.5})
            
        experiment_end = timer()

        #upon completing an epoch of training, collect required stats
        loss_pt = entropy.eval(feed_dict={x: randomizedX, y_: randomizedY}) #add keep_prob here??
    ...
    np.savetxt('./Q5c_dropout_test_accs.txt',np_test_accs)
    np.savetxt('./Q5c_dropout_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q5c_dropout.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q5c_dropout.png')
    ...
    plt.savefig('./Train Acc vs Test Acc_Q5c_dropout.png')
    plt.show()
