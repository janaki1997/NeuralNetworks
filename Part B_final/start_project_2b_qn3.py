...
def main():
  ...
  with tf.Session() as sess:
    ...
    np.savetxt('./Q3_test_accs.txt',np_test_accs)
    np.savetxt('./Q3_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q3.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q3.png')
    ...
    plt.savefig('./Train acc vc Test acc_Q3.png')
