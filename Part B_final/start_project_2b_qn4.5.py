...
def rnn_model(x):
  ...
  return logits, word_list, output_keep_prob
...
def main():
  ...
  logits, word_list, output_keep_prob = rnn_model(x)
  ...
  with tf.Session() as sess:
    ...
    np.savetxt('./Q5d_dropout_test_accs.txt',np_test_accs)
    np.savetxt('./Q5d_dropout_time.txt',np_times)
    ...
    plt.savefig('./Accuracy_vs_Epochs_Q5d_dropout.png')

    plt.figure(2)
    plt.plot(range(epochs), training_cost_pts)
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Training Cost')
    plt.savefig('./TrainingCost_vs_epochs_Q5d_dropout.png')
    ...
    plt.savefig('./Train Acc vs Test Acc_Q5d_dropout.png')
