...
def main():
    ...
    for size in batch_sizes:
        ...
        paras = mln(size)
        for i in range(epochs):
            time_to_update, test_accuracy, train_accuracy, c_err = mln(size)
            tr_accuracy.append(train_accuracy)
            te_accuracy.append(test_accuracy)
            if size == 32:
                err_batch.append(c_err)

        if size == 32:
            error_graph(err_batch, epochs)
               
        print('train acc:', tr_accuracy)
        accuracy_graphs(te_accuracy, tr_accuracy, size, epochs)
...
def accuracy_graphs(test_accuracy, train_accuracy, size, epochs):
