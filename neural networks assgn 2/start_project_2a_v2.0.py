...
import os
if not os.path.isdir('figures_partA_2'):
    print('creating the figures folder for part a')
    os.makedirs('figures_partA_2')
...
def main(numMaps1, numMaps2, counter):
    ...
    if numMaps1==30:
        with tf.Session() as sess:
            ...
            for e in range(epochs):
                ...
                print(X.shape)
                print(testX.shape)
                print(X.reshape(1,IMG_SIZE*IMG_SIZE*NUM_CHANNELS).shape)
    ...
    diname='./figures_partA_2'
    finalname=diname+'/test_acc'+'_'+str(numMaps1)+'.png'
    plt.figure(counter)
    ...
    finalname2=diname+'/training_cost'+'_'+str(numMaps1)+'.png'
    ...
    plt.axis('off')
    plt.imshow(X_show)
    plt.savefig('./figures_partA_2/p1b_2'+'_'+str(numMaps1)+'_'+str(numMaps1)+'.png')
    ...
    if (numMaps1==50 and numMaps2==60):
        ...
        for i in range(numMaps1):
            plt.subplot(j, 10, i+1); plt.axis('off'); plt.imshow(conv1_[0,:,:,i])
        plt.savefig('./figures_partA_2/conv1_maps1'+'_'+str(numMaps1)+'_'+str(numMaps1)+'.png')
        ...
        for i in range(numMaps1):
            plt.subplot(j, 10, i+1); plt.axis('off'); plt.imshow(pool1_[0,:,:,i])
        plt.savefig('./figures_partA_2/pool1_maps1'+'_'+str(numMaps1)+'_'+str(numMaps1)+'.png')
        ...
        plt.savefig('./figures_partA_2/conv2_maps1'+'_'+str(numMaps1)+'_'+str(numMaps1)+'.png')
        ...
        plt.savefig('./figures_partA_2/pool2_maps1'+'_'+str(numMaps1)+'_'+str(numMaps1)+'.png')

        diname='./figures_partA_2'
        finalname=diname+'/test_acc.png'
        plt.figure(200)
        acc_label='gradient descent'
        plt.plot(np.arange(epochs), test_acc, label=acc_label)
        plt.xlabel('epochs')
        plt.ylabel('test accuracy')
        plt.legend(loc='lower right')
        ...
        finalname2=diname+'/training_cost.png'
        plt.figure(201)
        cost_label='gradient descent'
        plt.plot(np.arange(epochs), training_cost, label=cost_label)
        plt.xlabel('epochs')
        plt.ylabel('training cost')
        plt.legend(loc='lower right')
